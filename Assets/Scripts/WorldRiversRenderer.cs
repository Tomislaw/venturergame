...
public class WorldRiversRenderer : MonoBehaviour
{
    ...
    public void Reload()
    {
        ...
        /*var*/
        meshes = GenerateMeshes(World.Regions).ToList();
    ...
    private IEnumerable<Mesh> GenerateMeshes(IEnumerable<Region> regions)
    {
        foreach (var region in regions)
            ...
            {
                yield return LineHelper.Liner.LineMesh(new List<Vector2>() { river.Edge.L, river.Edge.R }, Color, RiverWidth, 1 / (float)PixelsPerUnit);
                //var mesh = new Mesh();
                //var indices = new[] { 0, 1 };
